...
export const CanvasGate: React.FC<CanvasGate> = ({ canvas }) => {
  ...
  const [gatePosition, setGatePosition] = useState<GatePosition>({
    x: 0,
    y: 0,
    visible: false,
  });
  ...
  useEffect(() => {
    if (once) {
      ...
      canvas.addEventListener("click", () => {
        dispatch({ type: "SET_CLICKED", payload: cursorColorRef.current });
      });
  ...
  return (
    <div
      className={`gate ${gatePosition.visible ? "active" : ""}`}
      style={{ marginTop: gatePosition.y, marginLeft: gatePosition.x }}
    >
